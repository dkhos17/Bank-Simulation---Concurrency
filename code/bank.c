          ...
          int numWorkers)
{
  ...
  pthread_mutex_init(&(bank->nextDay), NULL);
  pthread_cond_init(&(bank->next), NULL);
  pthread_mutex_init(&(bank->check), NULL);
  pthread_mutex_init(&(bank->repTLock), NULL);
  return bank;
}
...
Bank_Balance(Bank *bank, AccountAmount *balance)
{
  ...
  for (unsigned int branch = 0; branch < bank->numberBranches; branch++) {
      sem_wait(&(bank->branches[branch].lockBranch));
  ...
  for(unsigned int branch = 0; branch < bank->numberBranches; branch++){
      sem_post(&(bank->branches[branch].lockBranch));
  }
