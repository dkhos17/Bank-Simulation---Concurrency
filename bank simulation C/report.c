...
{
  pthread_mutex_lock(&(bank->repTLock));
  ...
  if (amountAbs < reportingAmount) {
    pthread_mutex_unlock(&(bank->repTLock));
...
Report_DoReport(Bank *bank, int workerNum)
{
  ...
  bank->nWorkerLeft--;
  if(bank->nWorkerLeft > 0){
      pthread_mutex_unlock(&(bank->check));
      //sem_wait(&(bank->nextDay[workerNum]));
      //sem_wait(&(bank->nextDay));
      pthread_mutex_lock(&(bank->nextDay));
      pthread_cond_wait(&(bank->next), &(bank->nextDay));
      pthread_mutex_unlock(&(bank->nextDay));
      return 0;
  }else{
    Report *rpt = bank->report;
    ...
    if (rpt->numReports >= MAX_NUM_REPORTS) {
        ...
        pthread_mutex_lock(&(bank->nextDay));
        pthread_cond_broadcast(&(bank->next));
        pthread_mutex_unlock(&(bank->nextDay));
        
        bank->nWorkerLeft = numWorkers;
        //sem_post(&(bank->nextDay[workerNum]));
        pthread_mutex_unlock(&(bank->check));
        // We've run out of report storage for the bank
        return -1;
    }
