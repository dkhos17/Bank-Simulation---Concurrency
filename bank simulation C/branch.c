            ...
            AccountAmount initialAmount)
{
  ...
  for (int i = 0; i < numBranches; i++) {
    ...
    sem_init(&(bank->branches[i].lockBranch), 0, 1);
    pthread_mutex_init(&(bank->branches[i].lockBBalance) , NULL);
...
Branch_UpdateBalance(Bank *bank, BranchID branchID, AccountAmount change)
{
  ...
  pthread_mutex_lock(&(bank->branches[branchID].lockBBalance));
  AccountAmount oldBalance = bank->branches[branchID].balance; Y;
  bank->branches[branchID].balance = oldBalance + change; Y;
  pthread_mutex_unlock(&(bank->branches[branchID].lockBBalance));
  return 0;
}
